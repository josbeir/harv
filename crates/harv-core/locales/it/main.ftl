...
err-rate-limited = L’API Harvest è temporaneamente soggetta a limitazione. Riprova tra poco.
...
cli-config-unknown-setting = Impostazione sconosciuta: { $setting }. Impostazioni valide: access-token, account-id, aliases, cache-ttl, check-updates
cli-config-cache-ttl-invalid = cache-ttl deve essere un numero positivo
cli-config-check-updates-invalid = check-updates deve essere true o false
cli-config-locale-invalid = Invalid locale: { $value }. Supported: { $supported } (or empty/auto to reset)
cli-config-unknown-setting-set = Impostazione sconosciuta: { $setting }. Impostazioni valide: cache-ttl, locale, check-updates
...
tui-form-help-create = Tab: campo successivo │ Alt+Invio: invia │ Esc: annulla
tui-form-help-start = Tab: campo successivo │ Alt+Invio: avvia timer │ Esc: annulla
tui-form-help-edit = Tab: campo successivo │ Alt+Invio: salva │ Esc: annulla
...
tui-app-loading-entries = Caricamento voci orarie...
tui-app-loading-assignments = Caricamento dati progetto...

# TUI — Dashboard stats footer
tui-dash-projects = progetti
tui-dash-stats-total = totale
